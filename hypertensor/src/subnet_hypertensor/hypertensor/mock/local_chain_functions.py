...
from subnet_hypertensor.hypertensor.chain_data import (
    AllSubnetBootnodes,
    Attest,
    AttestEntry,
    ConsensusData,
...
from subnet_hypertensor.hypertensor.chain_functions import (
    EpochData,
    OverwatchEpochData,
    SubnetNodeClass,
    subnet_node_class_to_enum,
)
from subnet_hypertensor.hypertensor.config import BLOCK_SECS, EPOCH_LENGTH
from subnet_hypertensor.hypertensor.mock.mock_db import MockDatabase
...
class LocalMockHypertensor:
    def __init__(
        ...
        if insert_mock_overwatch_node:
            ...
            self.insert_overwatch_node(
                ...
                coldkey="",
                hotkey="",
                peer_id=PeerID.from_base58(
                    "12D3KooWHNjWMaBA4eW4KyrzPfduh6e7CQ91iqXfZ69ZLSNW1m6Q"
                ).to_base58(),
            )
    ...
    def get_overwatch_epoch_data(self) -> OverwatchEpochData:
        ...
        epoch_cutoff_block = (
            overwatch_epoch_length * overwatch_epoch + block_increase_cutoff
        )
        ...
        if current_block > epoch_cutoff_block:
            ...
            seconds_remaining_until_reveal = (
                epoch_cutoff_block - current_block
            ) * BLOCK_SECS
    ...
    def get_subnet_nodes_info_formatted(self, subnet_id: int) -> List["SubnetNodeInfo"]:
        ...
        try:
            ...
            logger.warning(
                f"[WARN] get_subnet_nodes_info_formatted error: {e}", exc_info=True
            )
            return []

    def get_overwatch_node_info_formatted(
        ...
        try:
            ...
            return OverwatchNodeInfo(
                ...
                account_overwatch_stake=int(
                    overwatch_node.get("account_overwatch_stake", 0)
                ),
            )
        except Exception as e:
            logger.warning(
                f"[WARN] get_overwatch_node_info_formatted error: {e}", exc_info=True
            )
            return None

    def get_all_overwatch_nodes_info_formatted(
        self,
    ) -> Optional[List["OverwatchNodeInfo"]]:
        try:
            overwatch_nodes = self.db.get_all_overwatch_nodes()
            all_overwatch_nodes = []
            ...
            for node_dict in overwatch_nodes:
                ...
                all_overwatch_nodes.append(
                    OverwatchNodeInfo(
                        ...
                        reputation=reputation,
                        account_overwatch_stake=int(
                            node_dict.get("account_overwatch_stake", 0)
                        ),
                    )
                )
            ...
            logger.warning(
                f"[WARN] get_all_overwatch_nodes_info_formatted error: {e}",
                exc_info=True,
            )
            return []
    ...
    def get_bootnodes(
        ...
        try:
            ...
            for bootnode_dict in bootnodes:
                subnet_bootnodes.append(
                    (bootnode_dict["peer_id"], bootnode_dict["bootnode"])
                )
            ...
            for node_dict in subnet_nodes:
                ...
                if node_class_enum.value > SubnetNodeClass.Registered.value:
                    ...
                    registered_bootnodes.append(
                        (node_dict["peer_id"], node_dict["bootnode"])
                    )
